...
def _load_thresholds():
    ...
    if os.path.exists(AE_THRESHOLD_PATH):
        ...
        print("threshold 데이터를 불러옵니다 : {AE_THRESHOLD_PATH}")
    else:
        _thresholds_cache = {}
    return _thresholds_cache
...
def load_model(category):
    model_path = os.path.join(WEIGHTS_DIR, f"autoencoder_{category}.pth")
...
def run_anomaly_inference(filepath, category):
    ...
    return loss, status, heatmap                       # tensor → PIL Image